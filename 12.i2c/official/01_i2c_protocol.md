...
以上老师就是主设备，学生就是从设备  
主模式：就是主CPU作为主机，向从机（挂zhi载器件）发送接收数据。  
...
* 开始信号（S）：SCL为高电平时，SDA 高 -> 低 跳变，开始传送数据。
* 结束信号（P）：SCL为高电平时，SDA 低 -> 高 跳变，结束传送数据。
* 响应信号(ACK)：接收器在接收到8位数据后，在第9个时钟周期(SCL第9次变高时)，拉低SDA
...
* 双方设备中，某个设备发送数据时，另一方怎样才能不影响SDA上的数据？
  ...
  上图通过上拉电阻拉高的方式，可以避免SDA两端同时1高1低烧坏电路  
